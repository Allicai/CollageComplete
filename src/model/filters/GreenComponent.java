...
/**
 * A class for a green component filter which changes the component of the red and blue to zero
 * and keeps the green.
 */
public class GreenComponent implements Filter {

  String name = "green-component";
    ...
    for (int i = 0; i < base.getHeight(); i++) { // getting a row of pixels
      for (int j = 0; j < base.getWidth(); j++) { // getting a pixel within the row
        if (!base.getRow(i).get(j).isTransparent()) { // transparency check
          ...
          base.getRow(i).get(j).setRgba(new int[]{0, bRgba[1], 0, bRgba[3]});
          // setting the rgba to only have the green component of the rgb and the same alpha value
        }
      }
    }
  }
